...
Xizmatlar o'z funksiyalari bilan sodda tilda gaplashadi. Masalan, broker `InsertStudentAsync` nomli metodni taqdim qilsa, xizmat qatlami bu metodning muqobili sifatida `AddStudentAsync` metodini qo'llaydi.

Umuman olganda, CRUD operatsiyalarining aksariyati xotira tilidan biznes logika tiliga o'girilishi kerak. Aslida, bu qoida xotirada saqlanmaydigan Navbat (Queues) kabi operatsiyalar uchun ham amal qiladi. Masalan, xotira tilida `PostQueueMessage` ko'rinishida nomlagan so'rov, biznes logika qatlamida `EnqueueMessage` ko‘rinishda bo'ladi.

CRUD - barcha tizimlarda umumiy operatsiya bo'lganligi sababli, biz CRUD operatsiyasini quyidagi ko'rinishda moslashtiramiz:
